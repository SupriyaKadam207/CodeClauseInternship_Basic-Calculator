...
def percentage():
    global equation
    try:
        num = float(equation)  # Convert equation to a number
        result = num / 100
        equation = str(result)  # Update the equation for chaining operations
        label_result.config(text=equation)  # Update the label with the percentage
    except:
        result = "error"  # Handle potential errors
...
Button(root,text="%", width=5, height=1, font=("arial",30,"bold"), bd=1,fg="#fff",bg="#2a2d36",command=lambda: show(percentage)).place(x=290,y=100)
